...
docker run --rm -it -v .:/workspace $GRADLE_IMAGE sh -c "cd /workspace && gradle build --build-cache -g .gradle.cache -x test"
if ! test $? -eq 0; then
...
find build/ -iregex ".+-SNAPSHOT[.]jar$" -print0 | xargs -I{} cp {} ./Dockerfile/data/app.jar 1>/dev/null 2>/dev/null
